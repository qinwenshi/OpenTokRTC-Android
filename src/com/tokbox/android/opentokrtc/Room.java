...
public class Room extends Session {
		...
		public void setPrimaryItem(ViewGroup container, int position,
			...
			for (Participant p : mParticipants) {
				if (p == object) {
					...
					mActivity.setAudioOnlyView(p.isSubscriberInAudioOnlyMode());
				} else {
					if (p.getSubscribeToVideo()) {
	...
	public Room(Context context, String roomName, String sessionId,
			String token, String apiKey, String username) {
	...
	protected void onConnected() {
		...
		presentText((mActivity.getResources()
				.getString(R.string.welcome_text_chat)));
		if (mPublisherName != null && !mPublisherName.isEmpty()) {
			sendChatMessage(mActivity.getResources().getString((R.string.nick))
					+ " " + mPublisherName);
		}
	}
	...
	protected void onStreamDropped(Stream stream) {
		...
		if (p != null) {
			...
			mParticipantConnection.remove(stream.getConnection()
					.getConnectionId());
			mPagerAdapter.notifyDataSetChanged();
	...
	protected void onSignalReceived(String type, String data,
		...
		Log.d(LOGTAG, "Received signal:" + type + " data:" + data
				+ "connection: " + connection);
	...
	protected void onError(OpentokError error) {
		super.onError(error);
		Toast.makeText(this.mContext, error.getMessage(), Toast.LENGTH_SHORT)
				.show();
	}
